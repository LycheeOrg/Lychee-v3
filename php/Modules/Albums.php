...
final class Albums {
	...
	public function get($public = true) {
		...
		// Albums query
		if ($public===false) $query = Database::prepare(Database::get(), 'SELECT id, title, public, visible, sysstamp, min_takestamp, max_takestamp, password FROM ? ' . Settings::get()['sortingAlbums'], array(LYCHEE_TABLE_ALBUMS));
		else                 $query = Database::prepare(Database::get(), 'SELECT id, title, public, visible, sysstamp, min_takestamp, max_takestamp, password FROM ? WHERE public = 1 AND visible <> 0 ' . Settings::get()['sortingAlbums'], array(LYCHEE_TABLE_ALBUMS));
